  ...
  constructor(props){
    ...
    this.state ={
      ...
      showAlert: false,
        ...
        type_source:props.match.params.type,
...
handleSubmit = async (event) => {
  ...
  const planaction = {
    ...
    id_scrap:this.state.id_scrap,
    id_rapportnc:this.state.id_rapportnc,
    id_supercontrole:this.state.id_supercontrole,
    id_suivifournisseur:this.state.id_suivifournisseur,
    id_suiviclient:this.state.id_suiviclient,
    id_dmpp:this.state.id_dmpp,
...
closeModal1 = () => this.setState({ showAlert: false });
   ...
   render(){
    if(this.state.did > 0 && this.state.type_source==="nc"){
       
      this.state.id_rapportnc=this.state.did;
  }
  else if(this.state.did > 0 && this.state.type_source==="dmpp"){
    this.state.id_dmpp=this.state.did;
  }  else if(this.state.did > 0 && this.state.type_source==="suiviclient"){
    this.state.id_suiviclient=this.state.did;
  }  else if(this.state.did > 0 && this.state.type_source==="suivifournisseur"){
    this.state.id_suivifournisseur=this.state.did;
  }  else if(this.state.did > 0 && this.state.type_source==="scrap"){
    this.state.id_scrap=this.state.did;
  } else if(this.state.did > 0 && this.state.type_source==="super"){
    this.state.id_supercontrole=this.state.did;
  }
        ...
        <div className="row">
                   ...
                   <Form.Group>
                      <div className="row">
                        ...
                        <label className="form-check-label">Departement</label>
                         ...
                         <select disabled className="form-control" name="origine" onChange={this.handleChange2} size="sm"  style={{backgroundColor: "#ecf2f8"}} >
          ...
          cols={5} required style={{ backgroundColor: "#ecf2f8"}} type="text"name="cause" onChange={this.handleChange4} size="sm"   />                         
                         ...
                         <Form.Group>
                          ...
                          <textarea className=" form-control" rows={5}
          cols={5}required style={{ backgroundColor: "#ecf2f8"}} type="text"name="action" onChange={this.handleChange10} size="sm"   />                                                    
                          </Form.Group>
                          </div>
                          </div> 
                          <div className="row">
                       ...
                       <Form.Group>
                         ...
                         { this.state.users.map( user =>( 
                            <option key={user.id_user} value={user.id_user}> {user.first_name} {user.last_name}</option>
                          ))} 
                           </select>                             
                           </Form.Group>
                         </div>
                       ...
                       <Form.Group>
                         ...
                         <Form.Control className=" form-control"required ={this.state.ttf} style={{ backgroundColor: "#ecf2f8"}} type="text"name="contol_effic" onChange={this.handleChange12} size="sm"   />                         
                           </Form.Group>
                         </div>
                         <div className="col-md-6">
                       <Form.Group>
                      ...
                      <Form.Group>
                         ...
                         <select required className="form-control" name="annul" onChange={this.handleChange14} size="sm"  style={{backgroundColor: "#ecf2f8"}} >
                              <option disabled value="" >ANNULER</option>
